...
import type { ToastType, ToastOptions, ToastFn } from '../types';
...
function Toast({ id, type = 'info', title, message, onDismiss }: ToastItem & { onDismiss: (id: number) => void }) {
  return (
    <motion.div
      layout
      ...
      className={`card flex items-start gap-3 p-4 min-w-72 max-w-sm border-l-4 ${BORDER[type]}`}
      role="alert"
      aria-live="polite"
    >
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = useCallback(({ type = 'info', title, message, duration = 4000 }: ToastOptions): number => {
  ...
  return (
    <ToastContext.Provider value={toast}>
      {children}
      <div className="fixed bottom-6 right-6 z-100 flex flex-col gap-2 pointer-events-none" aria-label="Notifications">
